...
class WideWeatherPrediction(WeatherPredictionHandler):
    ...
    def get_df(cls, df: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        for col in params['cols_to_transform']:
            df_pivoted = df.pivot_table(index=date_col,
                                        columns=(df
                                                 .groupby(date_col)
                                                 .cumcount()), # type: ignore
                                        values=col,
                                        aggfunc='sum').add_prefix(col + '_')
...
class OrderWeatherPrediction(WeatherPredictionHandler):
    ...
    def get_df(cls, df: pd.DataFrame, params: dict) -> pd.DataFrame:
        ...
        if params['descending_order'] != -1:
            df = (df
                  .groupby(params['grouby_col'], as_index=False)
                  .apply(lambda x: x.sort_values(by=params['col_idx'], ascending=False)
                  .iloc[params['descending_order']]))
        else:
             df = (df
                   .groupby(params['grouby_col'], as_index=False)
...
class FreshWeatherPrediction(WeatherPredictionHandler):
    ...
    def check(df: pd.DataFrame) -> None:
        ...
        mean_dict = {'2009-08-01 01:00:00': 1.88,
                    ...
                    '2012-06-25 00:00:00': 4.910} # Last date for test
        for key in mean_dict:
            diff = abs(float(df[df.date == key]['ws']) - mean_dict[key])
            if not diff < 0.01:
                raise AssertionError(f"Values are different for ws column for date {key}, diff={diff}")
...
class AggregatedWeatherPrediction(WeatherPredictionHandler):
    ...
    def get_df(cls, df: pd.DataFrame, params: dict):
        df = df.copy()
        df = cls.extend_df(df=df,
                           date_and_hors=params['extend_df'],
                           date_col=params['date_col'])
        divider = (params['fit_to_test_params']['mul']
        ...
        df = cls.aggregate_weather_predictions(df=df,
                                               date_col=params['date_col'],
                                               agg_params=params['agg_params'])
        if params['check']:
    ...
    def extend_df(df: pd.DataFrame,
        ...
        for elem in date_and_hors:
            ...
            extend_dict = {'date': df_copy[date_filter][date_col].unique(),
    ...
    @staticmethod
    def fit_to_test_df_structure(df: pd.DataFrame,
                                 fit_to_test_params: dict
                                 ) -> pd.DataFrame:
        df_copy = df.copy()
        ...
        for grouped_idx in range(0, int(len(df_copy) / grouped_length)):
            start_idx = (grouped_idx * grouped_length 
            ...
            for idx in range(len(start_hors[1:])):
                start_idxs.append(start_idx+mul*(end_hors[idx] - start_hors[idx] + 1))
            elems_for_start_end_filter = zip(fit_to_test_params['group_start_hors'],
    ...
    @staticmethod
    def aggregate_weather_predictions(df: pd.DataFrame,
                                      date_col: str,
                                      agg_params: dict) -> pd.DataFrame:
        df_copy = df.copy()
        ...
        cols_to_agg = df_copy.columns[
            ~df_copy.columns.isin(agg_params['exclude_cols'])
            ]
        df_copy = (df_copy[cols_to_agg] # type: ignore
    ...
    def check(df: pd.DataFrame, method: str) -> None:
        ...
        # NOTE: function to check:
        if method == 'mean':
            mean_dict = {'2009-08-01 01:00:00': 1.7425,
                        # First test date for second period + '2011-01-04 13:00:00'
                        # the same
            ...
            for key in mean_dict:
                diff = abs(float(df[df.date == key]['ws']) - mean_dict[key])
                if not diff < 0.01:
                    raise AssertionError(f"Values are different for ws column for date"
                                         f"{key}, diff={diff}")
            print("Status: SUCCESS")
        else:
